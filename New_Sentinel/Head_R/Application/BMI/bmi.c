...
int8_t BMI_Init(void)
{
  /* Status of api are returned to this variable. */
	...
	bmi_structure.state = BMI_DEINIT;
    return rslt;
}
...
float q0 = 0.707f, q1 = 0.0f, q2 = 0.707f, q3 = 0.0f;
...
/*加工过的欧拉角获取函数（方便PID调参）*/
void my_BMI_Get_EulerAngle(int16_t *pitch,int16_t *yaw ,short *p_ggy , short *p_ggz)
{
		
	//BMI_Get_RawData(short *ggx,short *ggy,short *ggz,short *aax,short *aay,short *aaz)
	short ggx,ggy,ggz,aax,aay,aaz;
		BMI_Get_RawData(&ggx, &ggy, &ggz, &aax, &aay, &aaz);
	short tem_yaw,tem_pit;
		...
		tem_pit = (int16_t)(-asin( 2 * q1 * q3 -2 * q0* q2)*57.3f + 180) * 22.7555f;
		tem_yaw = (int16_t)(atan2(2*(q1*q2 + q0*q3),q0*q0 +q1*q1-q2*q2 -q3*q3)*57.3f + 180)* 22.7555f;
		
		
		/*低通滤波*/
		*pitch = Low_Pass_Fliter(tem_pit,bmi_structure.yaw_angle , 1);
		*yaw   = Low_Pass_Fliter(tem_yaw,bmi_structure.pit_angle , 1);
		*p_ggy   = ggy;
		*p_ggz   = ggz;
...
int16_t bmi_init_cnt = 0;
float   Low_Pass_A_pit = 0.5;
float   Low_Pass_A_yaw = 0.5;
void BMI_Updata()
{		
	
	if(bmi_structure.state ==BMI_DEINIT && bmi_init_cnt++ > BMI_INIT_TIME_CNT)
	{
		bmi_structure.state = BMI_INIT;
	}
	
	BMI_Get_RawData(&gyrox, &gyroy, &gyroz, &accx, &accy, &accz);
	
	my_BMI_Get_EulerAngle(&bmi_structure.yaw_angle, &bmi_structure.pit_angle,NULL ,NULL);
	bmi_structure.yaw_gro = Low_Pass_Fliter(gyroy,bmi_structure.yaw_gro,Low_Pass_A_yaw);
	bmi_structure.pit_gro = Low_Pass_Fliter(gyroz,bmi_structure.pit_gro,Low_Pass_A_pit);
}
