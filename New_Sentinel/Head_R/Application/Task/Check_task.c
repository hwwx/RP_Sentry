...
void CheckTask(void const * argument)
{
	...
	{	
		...
		else if(Master_Head_structure.R_Head_status.status == DEV_OFFLINE)
		{
			LASER_OFF();
		}
		else
...
void GimbalTask(void const * argument)
{
	while(1)
	{
		...
		{
			gimbal_structure.base.target_pit = Master_Head_structure.Send_R_Head.target_pit;
			gimbal_structure.base.target_yaw = Master_Head_structure.Send_R_Head.target_yaw;
...
void H2MTask(void const * argument)
{
	while(1)
	{
		...
		{
			Master_Head_structure.From_R_Head.gimbal_mode = 1;
		}
		else
		{
			Master_Head_structure.From_R_Head.gimbal_mode = 0;
		}
		
		if(shoot_structure.base.status == Shoot_Online)
		{
			Master_Head_structure.From_R_Head.shoot_mode = 1;
		}
		else
		{
			Master_Head_structure.From_R_Head.shoot_mode = 0;
		}
		
		/*将当前的云台反馈给Master*/
		Master_Head_structure.From_R_Head.measure_pit = gimbal_structure.pitch->base_info->angle;
		Master_Head_structure.From_R_Head.measure_yaw = gimbal_structure.yaw->base_info->angle;
