...
void Check_One_Shoot(shoot_t * shoot)
{
	...
	{
		if(fabs(shoot->stir_wheel->base_info->angle_sum -shoot->stir_wheel->base_info->target_angle_sum) <= 20)//shoot_structure.stir_wheel->base_info->angle_sum -shoot_structure.stir_wheel->base_info->target_angle_sum
		{
...
{	
	if(fabs(shoot->pid_s_left->err[0]) <= 30)
	{
		shoot->pid_s_left->p = 20;
		shoot->pid_s_left->i = 0.01;
		shoot->pid_s_left->d = 0;
	}
	else
	{
		shoot->pid_s_left->p = 40;
		shoot->pid_s_left->i = 0;
		shoot->pid_s_left->d = 1;
	}
	
	if(fabs(shoot->pid_s_righ->err[0]) <= 30)
	{
		shoot->pid_s_righ->p = 20;
		shoot->pid_s_righ->i = 0.01;
		shoot->pid_s_righ->d = 0;
	}
	else
	{
		shoot->pid_s_righ->p = 40;
		shoot->pid_s_righ->i = 0;
		shoot->pid_s_righ->d = 1;
	}
	/*摩擦轮速度环PID*/
