...
void H2M_GETT_DATA( uint32_t id ,Master_Head_t * pack , uint8_t *rxBuf)
{
	switch (id)
	{
		...
		case GIMBAL_DATA_ID:
			...
			break;
		case MASTER_MODE_ID:
			pack->Send_L_Head.gimbal_mode = (uint8_t)rxBuf[4];
			pack->Send_R_Head.gimbal_mode = (uint8_t)rxBuf[5];
			pack->Send_L_Head.shoot_mode  = (uint8_t)rxBuf[6];
			pack->Send_R_Head.shoot_mode  = (uint8_t)rxBuf[7];
			break;
