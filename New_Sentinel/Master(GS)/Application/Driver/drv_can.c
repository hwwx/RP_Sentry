...
void CAN2_rxDataHandler(uint32_t canId, uint8_t *rxBuf)
{
	...
	else if(canId == 0x201)//2006
	{
		...
		if(L_shoot_structure.status == Single_Shoot || L_shoot_structure.status == Visin_Shoot)//仅单发计算总角度和，防止单发连发模式切换反转
	...
	else if(canId == 0x202)//2006
	{
		...
		if(R_shoot_structure.status == Single_Shoot || R_shoot_structure.status == Visin_Shoot)
