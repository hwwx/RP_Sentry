...
float Kp = 10.0f;    
float Ki = 0.f;
float halfT = 0.001f; //虽然任务周期是1但是结果好像是2 （单独开任务，提高优先级）
...
void my_BMI_Get_EulerAngle(int16_t *pitch,int16_t *yaw ,short *p_ggy , short *p_ggz)
{
	...
	short ggx,ggy,ggz,aax,aay,aaz;
		...
		yaw_9015_use = (atan2(2*(q1*q2 + q0*q3),q0*q0 +q1*q1-q2*q2 -q3*q3)*57.3f + 180)* 22.7555f;
		
		/*低通滤波*/
		*pitch = Low_Pass_Fliter(tem_pit,bmi_structure.pit_angle , 1);
		*yaw   = Low_Pass_Fliter(tem_yaw,bmi_structure.yaw_angle , 1);
		*p_ggy   = ggy;
		*p_ggz   = ggz;
...
void BMI_Updata()
{
		...
		bmi_structure.yaw_gro = Low_Pass_Fliter(gyroz,bmi_structure.yaw_gro,0.5);
		bmi_structure.pit_gro = Low_Pass_Fliter(gyroy,bmi_structure.pit_gro,0.5);
		
		/*记录上电角度*/
		if(get_bmi_data_time == 1000)
		{
