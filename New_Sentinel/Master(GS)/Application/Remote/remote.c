...
void rc_base_info_update(rc_base_info_t *info, uint8_t *rxBuf)
{
  ...
  {
	rc_structure.info->status = REMOTE_OFFLINE;
  }
  else
  {
...
uint32_t rc_offline_cnt;

void RC_HEART(rc_t *rc)
{
	rc->info->offline_cnt ++;
	
	if(rc->info->offline_cnt >= REMOTE_OFFLINE_CNT_MAX)
	{
		rc->info->status = REMOTE_OFFLINE;
		rc->info->offline_cnt = REMOTE_OFFLINE_CNT_MAX;
		
	}
	else
	{
		rc->info->status = REMOTE_ONLINE;
	}
	
//	if(rc_structure.info->status == REMOTE_OFFLINE)
//	{
//		rc_offline_cnt++;
//		if(rc_offline_cnt%800 <= 100)
//		{
//			BUZZ_Freq(50);
//		}
//		else if(rc_offline_cnt%800 <= 300 &&  rc_offline_cnt%800 >= 200)
//		{
//			BUZZ_Freq(50);
//		}
//		else
