...
void Chassis_Init(chassis_t *Chassis )
{
	...
	/*跟随PID*/
	PID_struct_init( &chassis_pid_follow_structure,POSITION_PID,8000,0,0.2,0,0);
...
void Chassis_Power_Limit(chassis_t * chassis)
{
	...
	{
		chassis->work_info.config.speed_max = 9000;
...
void Chassis_Speed_Limit(chassis_t *chassis)
{
	...
	Chassis.work_info.config.speed_max = 4000;
...
void Chassis_Speed_Calculating(chassis_t *chassis, int16_t front, int16_t right, int16_t cycle)
{
	...
	/*逆解算*/
	chassis->base_info.measure.cycle_speed = (chassis->motor_LF->base_info->speed +
											  chassis->motor_RF->base_info->speed +
											  chassis->motor_LB->base_info->speed +
											  chassis->motor_RB->base_info->speed) / 4.0;
		
	chassis->base_info.measure.right_speed = (chassis->motor_LF->base_info->speed +
											  chassis->motor_RF->base_info->speed -
											  chassis->motor_LB->base_info->speed -
											  chassis->motor_RB->base_info->speed) / 4.0;
	
	chassis->base_info.measure.front_speed = (chassis->motor_LF->base_info->speed -
											  chassis->motor_RF->base_info->speed -
											  chassis->motor_LB->base_info->speed +
											  chassis->motor_RB->base_info->speed) / 4.0;
...
/**
  ...
  * @Date    2023-7-05
**/
void Chassis_HeartBeat(chassis_t *chassis )
{
		
	/*电机掉线检测*/
	Motor_3508_HeartBeat(chassis->motor_LB);
	Motor_3508_HeartBeat(chassis->motor_RB);
	Motor_3508_HeartBeat(chassis->motor_LF);
	Motor_3508_HeartBeat(chassis->motor_RF);
...
/**
  ...
  * @Date    2023-7-05
**/
void Chassis_Control(chassis_t *chassis, int16_t* output_current)
{
	/*哪怕底盘电机有一个不在线也要转*/
...
void Chassis_Work(chassis_t *chassis)
{
	...
	/*底盘控制*/
	Chassis_Control(chassis,output_current);
}
