...
#define CHASSIS_SPEED_MAX   	(9000)	/*底盘最大速度默认值*/
#define CHASSIS_OUTPUT_MAX 		(13000) /*底盘最大输出默认值*/
...
#define SPEED_PID_I 0.35
#define SPEED_LIM_I 8000    //积分上限
#define SPEED_LIM_O 10000   //输出上限


#define c_abs(x) 					((x)>0? (x):(-(x)))


#define right_time_ (2500)      //2834mm
#define fornt_time_ (3000)	   //1458mm
...
typedef struct
{
  motor_3508_t *motor_LF;       		 /*!< 左前轮电机初始化
									@ref 在rp_motor_pack中初始化 */

  motor_3508_t *motor_RF;      		 /*!< 右前轮电机初始化
									@ref 在rp_motor_pack中初始化 */

  motor_3508_t *motor_LB;       		 /*!< 左后轮电机初始化
									@ref 在rp_motor_pack中初始化 */

  motor_3508_t *motor_RB;      		 /*!< 右后轮电机初始化
									@ref 在rp_motor_pack中初始化 */

  int16_t   *power_limit_buffer;   /*!< 裁判系统底盘功率限制的buffer，需要该变量地址
									@ref 由裁判系统发回 */
...
{	
	...
	void                (*init)(struct chassis_class_t *chassis);
...
/*底盘控制*/
void Chassis_ControlChassis_Control(chassis_t *chassis, int16_t* output_current);
