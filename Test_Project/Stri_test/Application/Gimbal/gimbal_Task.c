...
void gimbal_work()
{
	...
	if(car_structure.ctrl == RC_CAR)
	{
		...
		else if(car_structure.mode == follow_CAR  || car_structure.mode == two_CAR)//跟随模式
		{
		...
		else if(car_structure.mode == vision_CAR ||car_structure.mode == aim_CAR || car_structure.mode ==  shake_CAR)//视觉模式
		{
			/*比赛代码*/
			#if 1
